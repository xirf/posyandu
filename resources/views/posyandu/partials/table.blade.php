<table class="table">
    <thead class="bg-[#f5f7f9]">
        <tr>
            ...
            <th x-show="openedIndex > 2" @click="sortBy('lab_results.cholesterol')" class="w-20 cursor-pointer"> <span>{{ __('Cholesterol') }}</span> </th>
            <th x-show="openedIndex > 2" @click="sortBy('lab_results.hemoglobin')" class="w-20 cursor-pointer"><span class="tooltip tooltip-bottom" data-tip="{{ __('Hemoglobin') }}">{{ __('HB') }}</span></th>
            <th x-show="openedIndex > 2" @click="sortBy('lab_results.gda')" class="w-20 cursor-pointer"><span class="tooltip tooltip-bottom" data-tip="{{ __('Glucose Level') }}">{{ __('GDA') }}</span></th>
            <th x-show="openedIndex > 2" @click="sortBy('lab_results.ua')" class="w-48 cursor-pointer"><span class="tooltip tooltip-bottom" data-tip="{{ __('Urine Test') }}">{{ __('UA') }}</span></th>
            <th x-show="openedIndex > 3" @click="sortBy('family_planning')" class="w-20 cursor-pointer"><span class="tooltip tooltip-bottom" data-tip="{{ __('Family Planning') }}">{{ __('KB') }}</span></th>
            <th x-show="openedIndex > 3" @click="sortBy('complaints')" class="w-48 cursor-pointer"><span>{{ __('Complaints') }}</span></th>
            <th x-show="openedIndex > 3" @click="sortBy('diagnosis')" class="w-48 cursor-pointer"><span>{{ __('Diagnosic') }}</span></th>
            <th x-show="openedIndex > 3" @click="sortBy('diseases')" class="w-48 cursor-pointer"><span>{{ __('Disease') }}</span></th>
            <th x-show="openedIndex > 3" @click="sortBy('medication')" class="w-48 cursor-pointer"><span>{{ __('Medication') }}</span></th>
    </thead>
    <tbody>
        <template x-for="data in activeData.data" :key="data.id">
            <tr class="hover:bg-cyan-50" x-show="!isLoading && !data.hiddenBySearch">
                ...
                <td x-show="openedIndex > 2" x-text="data.lab_results.cholesterol"></td>
                <td x-show="openedIndex > 2" x-text="data.lab_results.hemoglobin"></td>
                <td x-show="openedIndex > 2" x-text="data.lab_results.gda"></td>
                <td x-show="openedIndex > 2" x-text="data.lab_results.ua"></td>
                <td x-show="openedIndex > 3" x-text="data.family_planning"></td>
                <td x-show="openedIndex > 3" > <div x-text="data.complaints" class="line-clamp-4"></div> </td> 
                <td x-show="openedIndex > 3" > <div x-text="data.diagnosis" class="line-clamp-4"></div> </td>
                <td x-show="openedIndex > 3" > <div x-text="data.diseases" class="line-clamp-4"></div> </td>
                <td x-show="openedIndex > 3" > <div x-text="data.medication" class="line-clamp-4"></div> </td>
            </tr>
        </template>
