...
<div class="w-full">
    ...
    <select id="gender" name="gender"
        ...
        placeholder="{{ __('Choose Gender') }}" required value="{{ old('gender', $medicalRecord->patient->gender) }}">
...
@foreach ($fields as $field => $label)
    <div class="w-full">
        ...
        <x-text-input id="{{ $field }}" name="{{ $field }}"
            ...
            class="mt-1 block w-full" value="{{ old($field, $medicalRecord->patient->$field) }}"
    ...
    <script>
        ...
        function initTomSelect() {
            ...
            tomSelect.addOption({
                ...
                nik: '{{ $medicalRecord->patient->nik }}',
                nik: '{{ $medicalRecord->patient->nik }}',
                gender: '{{ $medicalRecord->patient->gender }}',
                place_of_birth: '{{ $medicalRecord->patient->place_of_birth }}',
                birthdate: '{{ $medicalRecord->patient->birthdate }}',
                dukuh: '{{ $medicalRecord->patient->dukuh }}',
                rt: '{{ $medicalRecord->patient->rt }}',
                rw: '{{ $medicalRecord->patient->rw }}',
            });
            tomSelect.addItem('{{ $medicalRecord->patient->name }}');
        }
    </script>
@endPushOnce
